...
adm_bp = Blueprint('admin_import', __name__) 

UPLOAD_FOLDER = config['CSV_FOLDER']

@adm_bp.route('/importar-administrativos', methods=['GET', 'POST'])
def importar_administrativos():
    form = AdmImport()
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            ruta_csv = os.path.join(UPLOAD_FOLDER, nombre_archivo)
            archivo.save(ruta_csv)

            importar_desde_csv(ruta_csv, nivel=nivel)
            flash('Importación completada correctamente.', 'success')
        except Exception as e:
            flash(f'Error durante la importación: {str(e)}', 'danger')

        return redirect(url_for('admin_import.importar_administrativos'))

    return render_template('importar_administrativos.html', form=form)
